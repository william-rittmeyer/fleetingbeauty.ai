...
def artwork_create():
  ...
  output_titles = openai.ChatCompletion.create(
    ...
    messages=[{"role": "user", "content":
              'generate 9 titles of painting ideas about animals'
  }]
  )
  ...
  output_description = openai.ChatCompletion.create(
    model="gpt-3.5-turbo",
    messages=[{"role": "user", "content":
              'generate a painting about animals, ' + title + '. Make the description 4 sentences long. Start the description with the words, A painting'
  }]
  )
    ...
    while True:
      ...
      # run the function
        try:
          ...
          #ref = db.reference('')
          #ref.update({'url_animals': url_google})
          #ref.update({'painting_name_animals': A[1]})
