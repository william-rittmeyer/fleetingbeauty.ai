...
def artwork_create():
  ...
  output_titles = openai.ChatCompletion.create(
    ...
    messages=[{"role": "user", "content":
              'generate 9 titles of painting ideas about beautiful places'
  }]
  )
  ...
  output_description = openai.ChatCompletion.create(
    model="gpt-3.5-turbo",
    messages=[{"role": "user", "content":
              'generate a painting about beautiful places, ' + title + '. Make the description 4 sentences long. Start the description with the words, A painting'
  }]
  )
